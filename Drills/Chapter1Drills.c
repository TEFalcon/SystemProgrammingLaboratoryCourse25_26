...
/**
 * @name Exercise 1-15
 * @brief program prints a table of Fahrenheit-Celsius conversion table
 */
#include <stdio.h>
...
double FahrenheitToC(int degF);

int main()
{
    int i;
    double c;
    for (i = START_F; i < END_F; i += 10)
    {
        c = FahrenheitToC(i);
        printf("F: %4d C: %.2f\n", i, c);
    }
...
double FahrenheitToC(int degF)
{
    return 5 * ((degF - 32) / 9.0);
}

/**
 * @name Exercise 1-19
 * @brief program reverses the charecter order in the input one line at a time
 */
#include <stdio.h>

#define MAXLINE 1000

void Reverse(char s[], int indx); // prints the reverse of the string given
